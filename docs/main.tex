...
\begin{itemize}
 \item \textbf{Sentinel-2} (Level-2A; 10\,m spatial resolution;
 4 bands: B2, B3, B4, B8).
 \item \textbf{Landsat-8} (Collection 2, Level-2; 30\,m; 4 bands).
 \item \textbf{Landsat-9} (Collection 2, Level-2; 30\,m; 4 bands).
 \item \textbf{MODIS} (MOD09GA; 500\,m; 4 bands).
\end{itemize}

Patches are stratified into train (80\%), validation (10\%), and test
(10\%) splits by geographic tile to prevent spatial leakage.
The full corpus comprises $N_{\mathrm{test}} = $ -- test
patches distributed across -- geographic tiles
...
Level-2A cloud-probability layers of each sensor:
\[
 x_{hwc} =
 \begin{cases}
...
at pixel $(h, w)$ within a sliding window $\Omega$ of size $s \times s$:
\[
...
$\hat{X}_{hw} = \sum_i w_i X_i / \sum_i w_i$, where
$w_i = 1 / (d_i^p + \varepsilon)$ and $\varepsilon = 10^{-10}$ prevents
division by zero. The method is exact at observed pixels and produces
smooth reconstructions that decay to a global mean in large gap regions.
...
that $s(\mathbf{x}) = \sum_k w_k \, \varphi(\|\mathbf{x} - \mathbf{x}_k\|)$,
where $\varphi(r) = r^2 \ln r$ is the thin-plate spline basis. The
...
$\int (\nabla^2 u)^2 \, \mathrm{d}A$ subject to interpolation
constraints at valid pixels. The same downsampling strategy applies.
...
functional $\mathrm{TV}(u) + \frac{\lambda}{2}\sum_{(h,w) \notin \mathcal{G}}
(u_{hw} - \tilde{X}_{hw})^2$, where $\lambda{=}0.1$. The data-fidelity
term applies only to observed pixels (weight $= 1$); gap pixels receive
weight $= 0$. The TV term enforces piecewise-smooth reconstructions via
the $\ell_1$ norm of the image gradient. The algorithm runs for 100
fixed iterations with step sizes $\tau{=}\sigma{=}0.125$ and momentum
parameter $\theta{=}1$.
...
$\hat{X}(\mathbf{x}) = Z^{-1} \sum_{\mathbf{y}} w(\mathbf{x},\mathbf{y})\,
u(\mathbf{y})$, where
$w \propto \exp(-\|P_{\mathbf{x}} - P_{\mathbf{y}}\|^2 / h^2)$ and
$h = 0.1 \, \hat{\sigma}$ is the filter strength scaled by the estimated
noise level. Patches of size $7 \times 7$ are compared. After filtering,
known pixel values are restored exactly.

\paragraph{Exemplar-based (biharmonic).}
The biharmonic equation $\nabla^4 u = 0$ is solved within the gap
region, subject to boundary conditions at the interface between gap and
valid pixels. This fourth-order PDE minimises the bending energy of the
interpolating surface, producing $C^1$-continuous reconstructions that
smoothly extend the surrounding structure into the gap. The method is
parameter-free and relies on the implementation provided by
scikit-image.
...
\input{tables/robust-regression}
...
\Cref{tab:dl-results} reports mean \gls{PSNR} (bootstrap 95\% \gls{CI})
...
\begin{figure}[t]
 \centering
 \includegraphics[width=\linewidth]{figures/fig6_visual_examples_sentinel2}
 \caption{%
...
\begin{figure}[t]
 ...
 % Additional metric variants produced by generate_figures.py:
 % fig7_corr_heatmap_{rmse,sam,ssim}.{pdf,png}
 \caption{%
