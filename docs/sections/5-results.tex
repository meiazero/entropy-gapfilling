...
A \Cref{tab:inferential-summary-classical} complementa esse quadro com um teste global de diferenças em \gls{PSNR} entre métodos.
...
Ao mesmo tempo, entre os cinco métodos líderes em \gls{PSNR}, os contrastes par a par deixam de ser significativos após correção de Bonferroni.
...
Nos cenários sem ruído e com 40 dB, \gls{RBF} e Thin Plate Spline aparecem de forma recorrente entre os melhores valores de \gls{PSNR} e \gls{SSIM}.
...
A \Cref{tab:runtime-speed-classical} mostra que o ganho em qualidade de métodos como Ordinary Kriging, \gls{RBF} e Thin Plate Spline vem acompanhado de um aumento substancial no tempo de inferência por recorte.
...
Como a figura está restrita ao cenário sem ruído e usa medianas com dispersão por \gls{IQR}, sua leitura deve ser entendida como retrato operacional desse regime específico, e não como síntese de todos os níveis de ruído.
...
Isso é relevante porque evita uma interpretação superficial em que \gls{PSNR} global elevado mascararia erro excessivo em bandas específicas.
...
A \Cref{fig:classical-spectral-profile} reforça essa leitura ao resumir o \gls{RMSE} mediano por banda dos principais métodos clássicos.
...
Alguns algoritmos apresentam associação fraca ou próxima de zero em \gls{PSNR}, mas associação mais marcada em \gls{SSIM}, \gls{SAM} ou \gls{ERGAS}.
...
Segundo, Ordinary Kriging se destaca como solução particularmente robusta em cenários mais ruidosos, enquanto \gls{RBF}, Thin Plate Spline e Exemplar-Based se mostram muito fortes em qualidade absoluta sob condições menos adversas.
...
Ela apresenta os melhores valores de \gls{PSNR}, \gls{SSIM}, \gls{RMSE}, \gls{SAM} e \gls{ERGAS} em todos os níveis de ruído considerados, o que indica forte equilíbrio entre fidelidade radiométrica, preservação estrutural e coerência espectral.
...
Do ponto de vista inferencial, a \Cref{tab:inferential-summary-dl} mostra que as diferenças globais em \gls{PSNR} são estatisticamente detectáveis ($H=1745.81$, $p<10^{-300}$), mas com tamanho de efeito agregado pequeno ($\varepsilon^2=0.012$).
Entre os cinco modelos comparados, o contraste par a par mais forte envolve \gls{AE} e U-Net, ainda assim com efeito pequeno em magnitude ($d=-0.179$).
...
\gls{GAN} e \gls{ViT} formam um segundo patamar relativamente próximo entre si.
Eles não alcançam a qualidade absoluta da U-Net, mas também não colapsam para o nível de \gls{AE} e \gls{VAE}.
...
\gls{AE} e \gls{VAE}, por sua vez, mantêm papel analítico importante como baselines de gargalo latente, mostrando que compressão simples do contexto observado não é suficiente para atingir o melhor nível de fidelidade.
...
\gls{AE} e \gls{VAE} aparecem como os modelos mais rápidos e com melhor relação \gls{PSNR}/s, enquanto U-Net cobra um custo adicional em troca de qualidade superior.
...
\gls{GAN} e \gls{ViT} frequentemente exibem menor degradação percentual em vários cenários, mesmo permanecendo abaixo da U-Net em valor absoluto de \gls{PSNR}.
...
\gls{GAN} e \gls{ViT} preservam parte do desempenho relativo de forma mais estável, enquanto \gls{AE} e \gls{VAE} ocupam um regime inferior, porém regular.
...
As correlações entre entropia e \gls{PSNR} ou \gls{SSIM} tendem a ser mais claramente negativas para a maioria das arquiteturas, sugerindo que o aumento de complexidade espacial atua como fator de dificuldade relativamente consistente dentro do conjunto de modelos profundos analisado.
...
Segundo, essa liderança não implica melhor robustez relativa em todos os cenários, já que \gls{GAN} e \gls{ViT} exibem degradação percentual menor em várias condições.
Terceiro, \gls{AE} e \gls{VAE} permanecem úteis como referências eficientes e simples, mas não alcançam o mesmo nível de fidelidade das arquiteturas mais expressivas.
...
Se a prioridade recai sobre métodos clássicos interpretáveis com forte análise multicritério, Ordinary Kriging, \gls{RBF}, Thin Plate Spline e Exemplar-Based aparecem como candidatos centrais, cada um com vantagens e limitações específicas.
Se a prioridade recai sobre melhor qualidade absoluta no domínio Sentinel-2, a U-Net emerge como solução mais forte.
Se o foco se desloca para estabilidade relativa ou eficiência dentro do bloco de \gls{DL}, a decisão torna-se mais plural e abre espaço para \gls{GAN}, \gls{ViT}, \gls{AE} e \gls{VAE} conforme o objetivo operacional.
