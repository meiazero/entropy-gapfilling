"""Integration tests for RBFInterpolator using synthetic data."""
...
# Use 16x16 images for speed -- RBF solves a dense linear system.
SIZE = 16
CHANNELS = 4


def _make_method(**kwargs):
    defaults = {"kernel": "thin_plate_spline", "max_training_points": 500}
    defaults.update(kwargs)
    return get_interpolator("rbf", **defaults)
...
class TestRBFInterpolator:
    """Standard integration tests for radial basis function interpolation."""
    ...
    def test_kernel_variants(self, kernel: str) -> None:
        clean = _small_clean_3d()
        mask = _small_mask()
        degraded = make_degraded(clean, mask)
        method = _make_method(kernel=kernel, max_training_points=200)
        result = method.apply(degraded, mask)
        assert result.shape == degraded.shape
        assert np.all(np.isfinite(result))
