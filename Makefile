...
paper-assets: paper-generate-assets ## Copy generated tables and figures to docs/
	...
	if [ -d "paper_assets/tables" ]; then _found_tables=1; fi; \
	if [ -d "paper_assets/figures" ]; then _found_figures=1; fi; \
	[ "$$_found_tables" = "1" ] || printf "\033[33mWARN\033[0m no tables directory in paper_assets\n"; \
	[ "$$_found_figures" = "1" ] || printf "\033[33mWARN\033[0m no figures directory in paper_assets\n"; \
	uv run python scripts/select_paper_tables.py; \
	uv run python scripts/select_paper_figures.py
