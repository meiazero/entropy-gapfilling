...
experiment: preprocess-all ## Run full paper experiment (preprocess + run)
	...
	@echo "Generating PDF-only figures and LaTeX tables for paper_results"
...
experiment-quick: preprocess-all ## Run quick classical-only validation (1 seed, 1 patch)
	...
	@echo "Generating PDF-only figures and LaTeX tables for quick_validation"
...
.PHONY: preview
preview: preprocess-all ## Full preview: classical quick + all DL models quick + unified PDF figures
	...
	@echo "=== Step 4/4: Generating unified PDF preview figures and tables ==="
	@_DL_DIR=$$(dirname "$(CKPT_DIR)"); \
	rm -rf results/quick_validation/figures; \
...
paper-generate-assets: ## Generate canonical paper assets with PDF-only figures into docs/
	@echo "Generating PDF-only paper assets directly into docs/figures and docs/tables"
	...
	@uv run python scripts/generate_paper_figures_extra.py --output docs/figures
	@uv run python scripts/generate_tables.py --output docs/tables \
		--bootstrap-samples $(TABLE_BOOTSTRAP_SAMPLES)

.PHONY: paper-assets
paper-assets: paper-generate-assets ## Ensure canonical docs assets exist with PDF-only figures
	@echo "Paper assets refreshed in docs/figures and docs/tables (PDF-only figures)"

## NOTE: table filename normalization removed. LaTeX now references
## the descriptive filenames generated by scripts/generate_tables.py.

.PHONY: paper
paper: paper-assets ## Generate PDF-only assets and compile docs/main.tex -> docs/dist/draft.pdf
	@mkdir -p $(BUILD_DIR)
...
.PHONY: paper-only
paper-only: ## Compile paper using existing docs/ assets; figures must already be PDF-only
	@mkdir -p $(BUILD_DIR)
	@rm -f $(BUILD_DIR)/$(JOB_NAME).fdb_latexmk $(BUILD_DIR)/$(JOB_NAME).fls
	@latexmk -xelatex -cd -interaction=nonstopmode -halt-on-error \
		-jobname=$(JOB_NAME) -outdir=$(abspath $(BUILD_DIR)) $(TEX_SRC)
